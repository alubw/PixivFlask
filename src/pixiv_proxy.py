...
def ariadl(url, path, name):
    return subprocess.Popen(['/usr/bin/aria2c', url, '-d', path, '-o', name, '--allow-overwrite=true', '--timeout=300', '--max-tries=3']).wait()
...
# 图片下载并创建缩略图
def apiDownload(persent: str, api: pixivpy3.AppPixivAPI, item: dict) -> None:
    # item = {"illust": illust, "title": title,
    #         "count": count, "tags": tags, "url": url, "suffix": suffix}
    logger.info("[" + persent + "]尝试下载 " + str(item['illust']) + ':' + item['title'])
    # 检测PATH是否存在，不存在则创建
    # 当日图片根文件夹
    ...
    for i in range(item['count']):
        ...
        if not (os.path.exists(origName) and isValidImage(origName)):  # 若图片不完整则下载
            ...
            if ariadl(url, origPath, name) != 0:
                raise Exception('aria2下载失败')
            # 封面创建缩略图
